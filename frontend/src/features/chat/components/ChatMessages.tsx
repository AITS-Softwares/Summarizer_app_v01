import { BookOpen, Bot, LoaderCircle, Pencil, ShieldCheck } from 'lucide-react'
  ...
  if (!current || current.messages.length === 0) {
    return (
      <div className="my-auto flex flex-col items-center py-8 text-center">
        ...
        <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.2em] text-[#af7627]">AITS intelligence workspace</p>
        <h2 className="text-balance text-2xl font-semibold tracking-[-0.035em] text-[#241a11] sm:text-[31px]">Turn documents into clear answers</h2>
        <p className="mt-3 max-w-[570px] text-sm leading-6 text-[#7d6c56]">Add files or a folder, then ask for a summary, comparison, report, or any specific insight you need.</p>
        ...
        <div className="mt-7 flex items-center gap-2 text-[11px] font-medium text-[#99876e]">
          <ShieldCheck size={14} className="text-[#b47a2a]" />
          Your provider setting controls where analysis is processed
        </div>
      </div>
    )
  }
      ...
      {current.messages.map((message) => (
        <div key={message.id} className={`group/message flex gap-3 ${message.role === 'user' ? 'justify-end' : ''}`}>
          {message.role === 'assistant' && (
            <div className="mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[#d99a36] text-[#2c1d0c]"><Bot size={16} /></div>
          )}
          <div className="relative max-w-[82%]">
            <div className={message.role === 'user' ? 'rounded-2xl rounded-br-md bg-[#f1e3ce] px-4 py-3 text-sm leading-6' : 'max-w-[760px] pt-1 text-sm leading-7 text-[#4e4234]'}>
              {message.role === 'assistant'
                ...
                : message.content}
              <div className="mt-2 text-[9px] font-semibold uppercase tracking-[0.12em] text-[#a1917d]">{message.provider} · {formatDate(message.createdAtUtc)}</div>
            </div>
            {message.role === 'user' && (
